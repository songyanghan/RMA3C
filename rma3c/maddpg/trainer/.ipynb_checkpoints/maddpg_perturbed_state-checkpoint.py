...
class MADDPGPerturbedAgentTrainer(AgentTrainer):
    ...
    def perturb(self, obs):
        if self.args.noise_type != 0:
            return self.obs(self.addNoiseForOneAgent(obs[None]))
    ...
    def update(self, agents, t):
        ...
        perturbed_obs = self.ComputeTargetPerturbedObs(noise_obs_next_n, agents)
        ...
        for i in range(self.args.gda_step):
            # train p network
            cur_perturbed_obs = [agents[self.agent_index].a_debug['perturbed_obs'](noise_obs_n[self.agent_index])]
