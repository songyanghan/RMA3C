...
def p_train(make_obs_ph_n, obs_space_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
...
class PNMADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model, obs_shape_n, obs_space_n, act_space_n, agent_index, args, perturbed_trainers, local_q_func=False):
        self.name = name
        self.n = len(obs_shape_n)
        self.agent_index = agent_index
        self.args = args
        self.perturbed_trainers = perturbed_trainers
        if args.print_debug != 0:
            print("obs_shape_n: " + ' '.join([str(elem) for elem in obs_shape_n]))
            print("obs_space_n: " + ' '.join([str(elem) for elem in obs_space_n]))
            print("act_space_n: " + ' '.join([str(elem) for elem in act_space_n]))
        obs_ph_n = []
    ...
    def addNoiseForOneAgent(self, origin_obs):
        temp = np.array(origin_obs)
        noise = self.X.rvs(np.size(temp)) * self.args.d_value
        noise.shape = temp.shape
        return (temp+noise).tolist()
